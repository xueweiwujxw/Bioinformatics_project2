...
modelfn = 'output/model-data%s-%s_of_CNN.pth' % (seqid, c_time)

print('#building model ...')
print('#model: CNN(depth=6, width=128)')
...
class ConvBlock(nn.Module):
    def __init__(self, depth, numio):
        ...
        blk = [nn.Sequential(nn.Conv2d(numio, numio, kernel_size=3, padding=1),
                             nn.BatchNorm2d(numio), nn.ReLU())
               for i in range(depth)]
        self.blk = nn.ModuleList(blk)
...
class ConvNet(nn.Module):
    def __init__(self, depth, width):
        ...
        cnn0 = nn.Sequential(nn.Conv2d(numin, numout, kernel_size=3, padding=1, stride=2),
                      nn.BatchNorm2d(numout), nn.ReLU(),
                      ConvBlock(depth, numout))
...
#         embed = embed.sum(1) / (embed.size(1) - mask.unsqueeze(2).float().sum(1))
        mem = embed.unsqueeze(1)# .reshape(-1, 1, 8, 8)
        mem = self.net(mem)
        mem = self.flatten(mem)
        return self.out0(mem), self.out1(mem), self.out2(mem), mem
# end model

model = ConvNet(depth=6, width=128).cuda()
