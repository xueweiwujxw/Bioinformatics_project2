...
modelfn = 'output/model-data%s-%s_of_DenseNet.pth' % (seqid, c_time)

print('#building model ...')
print('#model: DenseNet(depth=2, width=64)')
...
class DenseBlock(nn.Module):
    def __init__(self, depth, numio):
        ...
        blk = [nn.Sequential(nn.Conv2d(numio*(i+1), numio, kernel_size=1, padding=0),
    ...
    def forward(self, x):
        for layer in self.blk[:-1]:
            x = pt.cat([x, layer(x)], 1)
        x = self.blk[-1](x)
        return x
...
model = DenseNet(depth=3, width=64).cuda()
