...
modelfn = 'output/model-data%s-%s_of_TransNet2_out4.pth' % (seqid, c_time)

print('#building model ...')
print('#model: TransNet2_out4(depth=6, width=512)')
...
class TransNet2(nn.Module):
    def __init__(self, depth, width, dropout = 0.1):
        ...
        self.out3 = nn.Linear(width, size3)
    def forward(self, x, mask):
        ...
        return self.out0(mem), self.out1(mem), self.out2(mem), self.out3(mem), mem
# end model
...
for epoch in range(numepoch1-1):
    ...
    for i, (x, m, y) in enumerate(trainloader):
        x = Variable(x).cuda()
        m = Variable(m).cuda()
        y0, y1, y2, y3 = Variable(y[:, 0]).cuda(), Variable(y[:, 1]).cuda(), Variable(y[:, 2]).cuda(), Variable(y[:, 3]).cuda()
        idx2 = combinations(range(x.size(0)), 2)
        yaln = pt.cat([y[i, :3] == y[j, :3] for i, j in idx2]).reshape([-1, 3]).float().cuda()
        yy0, yy1, yy2, yy3, yycode = model(x, m)
    ...
    for i, (x, m, y) in enumerate(validloader):
        ...
        m = Variable(m).cuda()
        y0, y1, y2, y3 = Variable(y[:, 0]).cuda(), Variable(y[:, 1]).cuda(), Variable(y[:, 2]).cuda(), Variable(y[:, 3]).cuda()
        yy0, yy1, yy2, yy3, _ = model(x, m)
        yy0, yy1, yy2, yy3 = pt.argmax(yy0, dim=1), pt.argmax(yy1, dim=1), pt.argmax(yy2, dim=1), pt.argmax(yy3, dim=1)

        acc0, acc1, acc2, acc3 = float(pt.sum(yy0 == y0)), float(pt.sum(yy1 == y1)), float(pt.sum(yy2 == y2)), float(pt.sum(yy3 == y3))
        acc01 = float(pt.sum((yy0 == y0) & (yy1 == y1)))
        acc012 = float(pt.sum((yy0 == y0) & (yy1 == y1) & (yy2 == y2)))
        acc0123 = float(pt.sum((yy0 == y0) & (yy1 == y1) & (yy2 == y2) & (yy3 == y3)))
        batch_acc.append([acc0, acc1, acc2, acc01, acc012, acc0123])
